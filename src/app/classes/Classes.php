...
class Classes extends Dbh{
    ...
    public function ShowStudentsInClass($profil, $name){
        try{
            $pdo = $this->connect();
            $sql = "SELECT pesel,imie,nazwisko,$profil FROM rekrutacja_uczen_tbl WHERE $profil > 0";
            ...
            if($row > 0){
                ...
                foreach($row as $row){
                    ...
                    echo "<td class='td_uczen col-200 td_10'>" . $row[$profil] . "</td>";
                    echo "</tr>";
                }
                echo "</table>";
                echo "</div>";
